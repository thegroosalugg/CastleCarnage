...
def invoice(who, what, where)
  ...
  tag = if item[:stat] == :attack || item[:stat] == :block # checks if item is attack/block
  ...
  pouch_item = what     == :pouch  ? "#{item[:phrase]} #{tag}" : "#{ITEMS.sample[:phrase]} +#{what} #{who[:emoji]}" # attaches correct tags
  pouch_trap = what     == :pouch  ? "#{item[:phrase]} #{tag}" : "#{TRAPS.sample[:phrase]} -#{what} #{who[:emoji]}"
